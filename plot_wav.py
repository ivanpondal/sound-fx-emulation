...
def analyze_pred_vs_actual(args):
    ...
    ax_orig.plot(abs(signal1_fft[:fft_valid_len1-1]), label=output_wav, color='red')
    ax_orig.set_title('Processed signal')
    fft_valid_len2 = int(len(signal2_fft)/2)
    ax_pred.plot(abs(signal2_fft[:fft_valid_len2-1]), label=pred_wav, color='green')
    ax_pred.set_title('Predicted signal')
    ax_merged.plot(abs(signal1_fft[:fft_valid_len1-1]), label='processed', color='red')
    ax_merged.plot(abs(signal2_fft[:fft_valid_len2-1]), label='predicted', color='green')
    ax_merged.legend(loc='upper right')
    ax_merged.set_xlabel("Frequency")
    ax_merged.set_ylabel("Amplitude")
    ax_orig.set_ylabel("Amplitude")
    ax_pred.set_ylabel("Amplitude")
    ax_orig.set_xlabel("Frequency")
    ax_pred.set_xlabel("Frequency")
